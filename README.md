...
# Откройте PowerShell в папке проекта и выполните: `powershell -File download\_nugets.ps1`

# Скрипт сам скачает все необходимые NuGet-пакеты в папку `NuGetLocal`.

# 3\. Соберите проект.  

# В той же папке выполните: `dotnet build` или просто откройте `RedPhoneDXConfig.sln` в Visual Studio и нажмите \*\*Сборка → Собрать решение\*\*.
